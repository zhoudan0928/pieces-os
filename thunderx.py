...
basicRequestHeaders_2 = {
    "x-requested-with": "com.thunder.downloader",
...
# 获取UA
def get_User_Agent(
    client_id, device_id, ua_key, timestamp, phoneModel, phoneBuilder, version
):
    UA = (
        "ANDROID-com.thunder.downloader/"
        + version
        + " protocolversion/200 accesstype/ clientid/"
        + client_id
        + " clientversion/"
        + version
        + " action_type/ networktype/WIFI sessionid/ deviceid/"
        + device_id
        + " providername/NONE devicesign/div101."
        + ua_key
        + " refresh_token/ sdkversion/2.0.3.203100 datetime/"
        + timestamp
        + " usrno/ appname/android-com.thunder.downloader session_origin/ grant_type/ appid/ clientip/ devicename/"
        + phoneBuilder.capitalize()
        + "_"
        + phoneModel.capitalize()
...
def get_ua_key(device_id):
    rank_1 = hashlib.sha1(
        (device_id + "com.thunder.downloader1appkey").encode("utf-8")
    ).hexdigest()
...
def get_init(client_id, captcha_token, device_id, User_Agent, action, meta):
    # print(meta)
    url = "https://xluser-ssl.xunleix.com/v1/shield/captcha/init"
...
def get_verification(client_id, captcha_token, email, device_id, User_Agent):
    url = "https://xluser-ssl.xunleix.com/v1/auth/verification"
...
def get_verify(client_id, verification_id, verification_code, device_id, User_Agent):
    url = "https://xluser-ssl.xunleix.com/v1/auth/verification/verify"
...
def signup(
    ...
    url = "https://xluser-ssl.xunleix.com/v1/auth/signup"
...
def random_version():
    version_list = [
        ...
        },
        {
            "v": "1.39.0",
            "algorithms": [
                {"alg": "md5", "salt": "e1d0IwHdz+CJLzskoFto8SSKobPWMwcz"},
                {"alg": "md5", "salt": "wUU7Rz/wpuHy"},
                {
                    "alg": "md5",
                    "salt": "dye78dKP7wgEFMebN/Z11VVPAAtueAVR3TcMFZPCO0F9mBQqbk/qpHy9Yqr0no",
                },
        ...
        },
        {
            "v": "1.40.0",
            "algorithms": [
                {"alg": "md5", "salt": "MNn/o2kDbAdap6iyA62c31+odfAXm"},
        ...
        },
        {
            "v": "1.41.0",
            "algorithms": [
                {"alg": "md5", "salt": "Wcpe+bWhLidcpKx+NbicS9tmSq8RbVTFk6Arf"},
...
def start(pop3flag):
    ...
    version_item = {
        "v": "1.06.0.2132",
        "algorithms": [
            {"alg": "md5", "salt": "kVy0WbPhiE4v6oxXZ88DvoA3Q"},
            {"alg": "md5", "salt": "lON/AUoZKj8/nBtcE85mVbkOaVdVa"},
            {"alg": "md5", "salt": "rLGffQrfBKH0BgwQ33yZofvO3Or"},
            {"alg": "md5", "salt": "FO6HWqw"},
            {"alg": "md5", "salt": "GbgvyA2"},
            {"alg": "md5", "salt": "L1NU9QvIQIH7DTRt"},
            {"alg": "md5", "salt": "y7llk4Y8WfYflt6"},
            {"alg": "md5", "salt": "iuDp1WPbV3HRZudZtoXChxH4HNVBX5ZALe"},
            {"alg": "md5", "salt": "8C28RTXmVcco0"},
            {"alg": "md5", "salt": "X5Xh"},
            {"alg": "md5", "salt": "7xe25YUgfGgD0xW3ezFS"},
            {"alg": "md5", "salt": ""},
            {"alg": "md5", "salt": "CKCR"},
            {"alg": "md5", "salt": "8EmDjBo6h3eLaK7U6vU2Qys0NsMx"},
            {"alg": "md5", "salt": "t2TeZBXKqbdP09Arh9C3"},
        ],
    }
    ...
    org_str = (
        client_id + version_item["v"] + "com.thunder.downloader" + device_id + timestamp
    )
    ...
    meta0 = {"email": email}
    action2 = get_init(
        client_id, "", device_id, user_agent, "POST:/v1/auth/verification", meta0
    )
    ...
    captcha_token = get_init(
        client_id, captcha_token, device_id, User_Agent, "POST:/v1/auth/signup", meta1
    )["captcha_token"]
    ...
    action9 = signup(
        ...
        name,
        password,
        action8["verification_token"],
        device_id,
        User_Agent,
    )
    user_id = action9["sub"]

    print("脚本执行成功！|迅雷X账号：", email, "|迅雷X密码：", password)
...
if __name__ == "__main__":
    ...
    try:
        print(
            "注意：启用了Pop3协议的正式邮箱可选择全自动接收验证码，目前支持的正式邮箱：Outlook、Hotmail、QQ、@126、Gmail、@163、Yahoo..."
        )
        pop3_flag = input(
            "是否启用自动接收验证码功能？【请输入'y'表示启用自动，否则为手动。】："
        )
        while True:
